...
# Create a new command in the bot tree named fun_fact
@bot.tree.command(name="fun_fact", description="Get a randomly generated fun fact!")
async def fun_fact(ctx: discord.Interaction): # Function to retrieve a fun fact using get_fact() in fun_fact.py
    await ctx.response.send_message(cmds.get_fact()) # Reply to the sender with the fun fact generated by fun_fact() in commands.py

# Create a new command in the bot tree named definitions
@bot.tree.command(name="definition", description="Get the definition of any word in the dictionary!")
